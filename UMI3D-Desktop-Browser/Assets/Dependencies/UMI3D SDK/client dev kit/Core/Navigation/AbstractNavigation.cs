...
namespace umi3d.cdk
{
    ...
    public abstract class AbstractNavigation : MonoBehaviour
    {
        ...
        /// <summary>
        /// User position on the ground.
        /// </summary>
        protected abstract Vector3 GetUserPosition();

        /// <summary>
        /// User rotation.
        /// </summary>
        protected abstract Quaternion GetUserRotation();
        ...
        public void Teleport(TeleportDto data)
        {
            ...
            if (data.Rotation != null)
            {
                ...
                world.position = (GetUserPosition() - (Vector3)data.Position);
                Vector3 rotation = ( Quaternion.Inverse(GetUserRotation()) * (Quaternion)data.Rotation ).eulerAngles;
                world.RotateAround(GetUserPosition(),Vector3.right, -rotation.x);
                world.RotateAround(GetUserPosition(), Vector3.up, -rotation.y);
                world.RotateAround(GetUserPosition(), Vector3.forward, -rotation.z);
            }
            else
            {
                world.position = (GetUserPosition() - world.rotation * (Vector3)data.Position);
            }
        }
