...
namespace umi3d.cdk.menu.view
{
    public class MenuDisplayManager : MonoBehaviour
    {
        ...
        AbstractMenuDisplayContainer CreateSubMenu(AbstractMenuDisplayContainer container, AbstractMenu subMenu, int containerDepth)
        {
            if (!menuToDisplayer.ContainsKey(subMenu))
            {
                ...
                UnityAction<AbstractMenuItem> action = (item) =>
                {
                    ...
                    {
                        if (item is AbstractMenu) CreateSubMenu(currentSubContainer, item as AbstractMenu, containerDepth + 1);
        ...
        {
            if (!itemToDisplayer.ContainsKey(item))
            {
