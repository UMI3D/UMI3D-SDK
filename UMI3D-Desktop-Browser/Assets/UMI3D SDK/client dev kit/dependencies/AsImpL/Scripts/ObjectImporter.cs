...
namespace AsImpL
{
    ...
    public class ObjectImporter : MonoBehaviour
    {
        ...
        public void ImportFile(Resource resource, Transform parentObject, ImportOptions options)
        {
            buildOptions = options;
            StartCoroutine(ImportFileAsync(resource, parentObject));
        }
        ...
        private IEnumerator ImportFileAsync(Resource resource, Transform parentObject)
        {
            string absolutePath = resource.Url;
            ...
            if (importAssets)
            {
                ...
                loader.buildOptions = buildOptions;
                yield return StartCoroutine(loader.Load(newName, resource, parentObject));
        ...
        public void ImportModelAsync(string objName, Resource resource, Transform parentObj, ImportOptions options)
        {
            if (loaderList == null)
            {
            ...
            string absolutePath = resource.Url.Contains("//") || resource.Url.Contains("http") ? resource.Url : Path.GetFullPath(resource.Url);
            absolutePath = absolutePath.Replace('\\', '/');

            //string absolutePath = resource.Url.Contains("//") ? resource.Url : Path.GetFullPath(resource.Url);
            //absolutePath = absolutePath.Replace('\\', '/');
            ...
            allLoaded = false;
            StartCoroutine(loader.Load(objName, resource, parentObj));
        }
