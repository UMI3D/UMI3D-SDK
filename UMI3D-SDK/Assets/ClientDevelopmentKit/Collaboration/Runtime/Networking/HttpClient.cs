...
namespace umi3d.cdk.collaboration
{
    ...
    public class HttpClient
    {
        ...
        /// <param name="callback">Action to be call when the request succeed.</param>
        /// <param name="onError">Action to be call when the request fail.</param>
        public void SendPostUpdateIdentity(Action callback, Action<string> onError)
        {
            Action<UnityWebRequest> action = (uwr) =>
            {
                //res ?
        ...
        /// <param name="callback">Action to be call when the request succeed.</param>
        /// <param name="onError">Action to be call when the request fail.</param>
        public void SendPostLogout(Action callback, Action<string> onError)
        {
            Action<UnityWebRequest> action = (uwr) =>
            {
                //res ?
        ...
        public void SendGetPublic(string url, Action<byte[]> callback, Action<string> onError)
        {
            ...
            client.StartCoroutine(_GetRequest(url, action, onError, false));
        }
        ...
        /// <param name="callback">Action to be call when the request succeed.</param>
        /// <param name="onError">Action to be call when the request fail.</param>
        public void SendGetPrivate(string url, Action<byte[]> callback, Action<string> onError)
        {
            Action<UnityWebRequest> action = (uwr) =>
            {
                var res = uwr.downloadHandler.data;
            ...
            client.StartCoroutine(_GetRequest(url, action, onError, true));
        }
        #endregion
        ...
        /// <param name="callback">Action to be call when the request succeed.</param>
        /// <param name="onError">Action to be call when the request fail.</param>
        public void SendPostJoin(JoinDto join, Action<EnterDto> callback, Action<string> onError)
        {
            Action<UnityWebRequest> action = (uwr) =>
            {
                var res = uwr.downloadHandler.data;
        ...
        /// <param name="onError">Action to be call when the request fail.</param>
        /// <returns></returns>
        IEnumerator _GetRequest(string url, Action<UnityWebRequest> callback, Action<string> onError, bool UseCredential = false, int tryCount = 0)
        {
