    ...
    public class UserAvatar : MonoBehaviour
    {
        ...
        private void Update()
        {
            foreach (var item in bounds)
            {
                if (item.obj != null)
                {
                    item.obj.position = UMI3DClientUserTracking.Instance.GetComponentInChildren<Animator>().GetBoneTransform(item.bonetype.Convert().GetValueOrDefault()).TransformPoint(item.offsetPosition);
                    item.obj.rotation = UMI3DClientUserTracking.Instance.GetComponentInChildren<Animator>().GetBoneTransform(item.bonetype.Convert().GetValueOrDefault()).rotation * item.offsetRotation;
                }
            }
