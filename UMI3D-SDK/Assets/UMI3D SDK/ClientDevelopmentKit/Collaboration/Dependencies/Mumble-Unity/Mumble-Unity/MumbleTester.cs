...
public class MumbleTester : MonoBehaviour {
    ...
    IEnumerator UpdateEditorGraph()
    {
        ...
        while (true)
        {
            ...
            long numSentThisSample = mumbleClient.NumUDPPacketsSent - numPacketsSent;
            long numRecvThisSample = mumbleClient.NumUDPPacketsReceieved - numPacketsReceived;
            long numLostThisSample = mumbleClient.NumUDPPacketsLost - numPacketsLost;
	...
	void Update () {
        if (!mumbleClient?.ReadyToConnect ?? true)
            return;
        if (Input.GetKeyDown(KeyCode.S))
        {
            mumbleClient.SendTextMessage("This is an example message from Unity");
            print("Sent mumble message");
        }
        if (Input.GetKeyDown(KeyCode.J))
        {
            print("Will attempt to join channel " + ChannelToJoin);
            mumbleClient.JoinChannel(ChannelToJoin);
        }
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            print("Will join root");
            mumbleClient.JoinChannel("Root");
        }
        if (Input.GetKeyDown(KeyCode.C))
        {
            print("Will set our comment");
            mumbleClient.SetOurComment("Example Comment");
        }
        if (Input.GetKeyDown(KeyCode.B))
        {
            print("Will set our texture");
            byte[] commentHash = new byte[] { 1, 2, 3, 4, 5, 6 };
            mumbleClient.SetOurTexture(commentHash);
        }
