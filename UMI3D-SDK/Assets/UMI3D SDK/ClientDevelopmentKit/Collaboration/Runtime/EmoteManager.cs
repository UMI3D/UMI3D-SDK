    ...
    public class EmoteManager : inetum.unityUtils.SingleBehaviour<EmoteManager>
    {
            ...
            foreach (UMI3DEmoteDto emoteDtoInConfig in emoteConfigDto.emotes)
            {
                if (emoteDtoInConfig is not null)
                {
                    var emote = new Emote()
                    {
                        ...
                        icon = defaultIcon,
                        uiOrder = i++, // emotes are always displayed in the order they were in the emote configuration
        ...
        private async void LoadFile(UMI3DEmoteDto emoteRefInConfig, Emote emote)
        {
            try
            {
                ...
                var image = await UMI3DResourcesManager.LoadFile(emoteConfigDto.id,
        ...
        public void ResetEmoteSystem()
        {
            ...
            UMI3DEnvironmentLoader.Instance.onEnvironmentLoaded.RemoveListener(LoadEmotes);
        ...
        {
            var animation = UMI3DEnvironmentLoader.Instance.GetEntityObject<UMI3DAbstractAnimation>(playingEmote.AnimationId);
