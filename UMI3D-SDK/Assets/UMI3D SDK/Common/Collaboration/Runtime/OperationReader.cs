    ...
    public class OperationReader : MonoBehaviour
    {
        ...
        List<ByteContainer> substring(string containerString)
        {
            ...
            while (m.Success && m.Groups.Count >= 3)
            {
                ...
                if (ignorePos)
                    result.AddRange(pss.Zip(lss, (pos, len) => (pos, len)).Zip(bss, (a, b) => new ByteContainer(0, 0, b, new UMI3DVersion.Version(UMI3DVersion.version))));
                else
                    result.AddRange(pss.Zip(lss, (pos, len) => (pos, len)).Zip(bss, (a, b) => new ByteContainer(0, 0, b, new UMI3DVersion.Version(UMI3DVersion.version)) { position = a.pos, length = a.len }));
                m = m.NextMatch();
            }
            return result;
        }
