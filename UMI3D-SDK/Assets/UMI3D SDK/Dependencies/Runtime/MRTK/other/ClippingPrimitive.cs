    ...
    public abstract class ClippingPrimitive : MonoBehaviour, IMaterialInstanceOwner
    {
        ...
        public bool UseOnPreRender
        {
            ...
            set
            {
                ...
                {
                    cameraMethods = GetOrAddComponent<CameraEventRouter>(CameraCache.Main.gameObject);
                }
        ...
        public void AddRenderer(Renderer _renderer)
        {
            if (_renderer != null)
            {
                ...
                ToggleClippingFeature(GetOrAddComponent<MaterialInstance>(_renderer.gameObject).AcquireMaterials(this), gameObject.activeInHierarchy);
                IsDirty = true;
            }
        }

        private T GetOrAddComponent<T>(GameObject mono) where T:Component
        {
            var c = mono.GetComponent<T>();
            if (c == null)
                c = mono.gameObject.AddComponent<T>();
            return c;
        }
        ...
        protected void ToggleClippingFeature(bool keywordOn)
        {
            if (renderers != null)
            {
                for (var i = 0; i < renderers.Count; ++i)
                {
                    ...
                    if (_renderer != null)
                    {
                        ToggleClippingFeature(GetOrAddComponent<MaterialInstance>(_renderer.gameObject).AcquireMaterials(this), keywordOn);
                    }
                }
            }
        }
